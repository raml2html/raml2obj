...
describe('raml2obj', () => {
  describe('consistent-examples.raml', () => {
    ...
    before((done) => {
      raml2obj.parse('test/consistent-examples.raml').then((result) => {
        obj = result;
        done();
      }, (error) => {
    ...
    it('all examples should have consistent representations', () => {
      ...
      assert.strictEqual(example.description, 'This descriptive header example');
      assert.strictEqual(example.value, 'Red baloons are known to be the colour red.');

      assert.strictEqual(obj.resources[3].relativeUri, '/UriExample/{Id}/{Mood}');
